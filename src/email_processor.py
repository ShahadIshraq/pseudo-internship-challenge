...
class EmailProcessor:
    ...
    def filter_emails(self, emails: list[Email]) -> list[Email]:
        ...
        filtered_emails = list(filter(lambda email:
                                    all(re.search(rf"{keyword}\w*", email.subject, re.IGNORECASE)
...
Hiring Team"""
    ...
    def process_emails(self) -> dict:
        ...
        def send_single_email(email):
            ...
            return self.gmail_client.send_email(email.sender, "Re: Pseudo-Internship Application" + email.subject, response_body)

        with concurrent.futures.ThreadPoolExecutor(max_workers=50) as executor:
            future_to_email = {executor.submit(send_single_email, email): email for email in filtered_emails}
